...
def validate_expert_reload_support(model: torch.nn.Module) -> None:
    """Fail before recovery mutates topology when a quant scheme is unsupported.

    Fault-tolerance scale-down currently reloads unquantized and W8A8 Dynamic
    expert slots. W4A8 and other schemes need scheme-specific reconstruction
    of their packed weights and scales, so they are rejected explicitly.
    """
    ...
    if unsupported:
        raise NotImplementedError(
            "[FT] scale_down expert reload supports only unquantized and "
            "W8A8 Dynamic MoE weights; unsupported quant method(s): "
            f"{', '.join(sorted(unsupported))}. W4A8 must be disabled for "
            "fault-tolerance scale-down."
        )
