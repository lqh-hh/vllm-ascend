...
SCALE_DOWN_ENGINE_RECOVERY_TIMEOUT_S = 300
INFERENCE_REQUEST_TIMEOUT_S = 120
DRIVING_REQUEST_TIMEOUT_S = 10
...
# original (so the all_reduce completes), skips startup/idle dummy batches,
# and raises after a real serving step returns.
...
if _SPEC:
    ...
    def _patch(m):
        ...
        def _wrapped(*args, **kwargs):
            ...
            num_reqs = bound.arguments.get("num_reqs", 0)
            if dp_rank == _RANK and num_reqs > 0:
                _steps[0] += 1
                if _steps[0] == _STEP:
                    raise RuntimeError(
...
def _ft_server_args(*, enable_eplb: bool) -> list[str]:
    engine_recovery_timeout_s = (
        SCALE_DOWN_ENGINE_RECOVERY_TIMEOUT_S if enable_eplb else ENGINE_RECOVERY_TIMEOUT_S
    )
    args = [
        "--dtype",
        "bfloat16",
...
class FTServerManager:
    ...
    def __enter__(self) -> list[tuple[RemoteOpenAIServer, list[str]]]:
        visible_devices = [
            device.strip()
            for device in os.getenv("ASCEND_RT_VISIBLE_DEVICES", "").split(",")
            if device.strip()
        ]
        if visible_devices and len(visible_devices) < self.dp_size:
            raise ValueError(
                f"FT test requires {self.dp_size} NPUs, but only "
                f"{len(visible_devices)} are listed in ASCEND_RT_VISIBLE_DEVICES."
            )
        if not visible_devices:
            visible_devices = [str(rank) for rank in range(self.dp_size)]
            ...
            def start_server(r: int, sargs: list[str]) -> None:
                try:
                    server = RemoteOpenAIServer(
                        ...
                        auto_port=False,
                        env_dict={
                            "ASCEND_RT_VISIBLE_DEVICES": visible_devices[r],
...
def _ft_manager(*, enable_eplb: bool) -> FTServerManager:
    return FTServerManager(
        MODEL_NAME,
        DP_SIZE,
        base_server_args=_ft_server_args(enable_eplb=enable_eplb),
        tp_size=1,
    )
...
def _complete(client, timeout_s: int = INFERENCE_REQUEST_TIMEOUT_S) -> Any:
...
def _driving(*servers: RemoteOpenAIServer):
    ...
    def _drive(server):
        ...
        while not stop.is_set():
            with contextlib.suppress(Exception):
                _complete(client, timeout_s=DRIVING_REQUEST_TIMEOUT_S)
            time.sleep(0.2)
...
def _assert_ft_recovered_and_serving(
    servers: tuple[RemoteOpenAIServer, ...],
    request_id: str,
    deadline_s: int,
) -> None:
    """Wait for FT recovery, failing early if any engine reports an error."""
    pending = dict(enumerate(servers))
    latest_statuses: list[dict[str, Any] | None] = [None] * len(servers)
    start = time.time()
    while pending and time.time() - start < deadline_s:
        for index, server in list(pending.items()):
            with contextlib.suppress(Exception):
                for engine_status in _get_ft_status(server)["engines"]:
                    latest_statuses[index] = engine_status
                    if (
                        engine_status.get("last_ft_request_id") == request_id
                        and engine_status.get("ft_error")
                    ):
                        pytest.fail(
                            f"FT recovery failed on survivor {index}: "
                            f"{engine_status['ft_error']}"
                        )
    ...
    assert not pending, (
        f"Survivors {sorted(pending)} did not recover within {deadline_s}s; "
        f"latest statuses: {latest_statuses}"
    )
    _in_parallel(lambda server: _complete(server.get_client()), servers)
...
def test_injected_fault_retry_recovers_all_ranks(monkeypatch, tmp_path):
    ...
    with _ft_manager(enable_eplb=False) as servers:
...
def test_worker_kill_scale_down_recovers_survivors():
    ...
    """
    with _ft_manager(enable_eplb=True) as servers:
        assert len(servers) == DP_SIZE
        ...
        _assert_ft_recovered_and_serving(
            survivors,
            request_id=scale_down_request_id,
